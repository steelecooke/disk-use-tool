...
process_day() {
  ...
  done < <(find / -type d -maxdepth $DEPTH -newermt "${DATE}T00:00:00" ! -newermt "${DATE}T23:59:59" -print0 2>/dev/null)
...
END_DATE=$(date +%Y-%m-%d)
START_DATE=$(date -v-30d -j -f "%Y-%m-%d" "$END_DATE" +%Y-%m-%d)

# Process each day in the date range
while [[ "$START_DATE" < "$END_DATE" ]]; do
  process_day "$START_DATE"
  START_DATE=$(date -v+1d -j -f "%Y-%m-%d" "$START_DATE" +%Y-%m-%d)
done
